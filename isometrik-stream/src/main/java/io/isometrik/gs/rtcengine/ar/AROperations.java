...
public class AROperations {
  ...
  private final ArrayList<AREffect> masks;
  private final ArrayList<AREffect> effects;
  private final ArrayList<AREffect> filters;
...
//      isometrik.getAREngine().switchEffect(slot, path);
  ...
  public void reapplyFilters() {
    for (int i = 0; i < masks.size(); i++) {
      if (masks.get(i).isSelected()) {
        applyFilter(SLOT_MASKS, masks.get(i).getPath());
        break;
      }
    }
    for (int i = 0; i < effects.size(); i++) {
      if (effects.get(i).isSelected()) {
        applyFilter(SLOT_EFFECTS, effects.get(i).getPath());
        break;
      }
    }
    for (int i = 0; i < filters.size(); i++) {
      if (filters.get(i).isSelected()) {
        applyFilter(SLOT_FILTERS, filters.get(i).getPath());
        break;
      }
    }
  }
