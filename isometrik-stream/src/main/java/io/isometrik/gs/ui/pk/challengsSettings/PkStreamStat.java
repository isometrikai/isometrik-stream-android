 ...
 * @see CommonResponse
 */
public class PkStreamStat {

  /**
   * Validate params.
   *
   * @param pkStreamStatQuery the fetch users request query
   ...
   * @see Gson
   */
  public void validateParams(PkStreamStatQuery pkStreamStatQuery,
                             @NotNull final CompletionHandler<PkStatsResponse> completionHandler,
                             @NotNull RetrofitManager retrofitManager, @NotNull IMConfiguration imConfiguration,
                             @NotNull final BaseResponse baseResponse, final @NotNull Gson gson) {

    String searchTag = pkStreamStatQuery.getSearchTag();

    if (searchTag != null && searchTag.isEmpty()) {
      completionHandler.onComplete(null, IsometrikErrorBuilder.IMERROBJ_SEARCH_TAG_INVALID_VALUE);
    } else {

      String userToken = pkStreamStatQuery.getUserToken();
      ...
      if (userToken == null || userToken.isEmpty()) {
        ...
        bodyParams.put("streamId", pkStreamStatQuery.getStreamId());

        Call<PkStatsResponse> call =
                retrofitManager.getUserService().fetchPkStreamStat(headers, bodyParams);

        call.enqueue(new Callback<PkStatsResponse>() {
          @Override
          public void onResponse(@NotNull Call<PkStatsResponse> call,
                                 @NotNull Response<PkStatsResponse> response) {
          ...
          @Override
          public void onFailure(@NotNull Call<PkStatsResponse> call, @NotNull Throwable t) {
