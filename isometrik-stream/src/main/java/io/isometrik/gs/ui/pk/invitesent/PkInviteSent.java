 ...
 * @see CommonResponse
 */
public class PkInviteSent {

  /**
   * Validate params.
   *
   * @param pkInviteSentQuery the fetch users request query
   ...
   * @see Gson
   */
  public void validateParams(PkInviteSentQuery pkInviteSentQuery,
                             @NotNull final CompletionHandler<CommonResponse> completionHandler,
                             @NotNull RetrofitManager retrofitManager, @NotNull IMConfiguration imConfiguration,
                             @NotNull final BaseResponse baseResponse, final @NotNull Gson gson) {

    String searchTag = pkInviteSentQuery.getSearchTag();

    if (searchTag != null && searchTag.isEmpty()) {
      completionHandler.onComplete(null, IsometrikErrorBuilder.IMERROBJ_SEARCH_TAG_INVALID_VALUE);
    } else {

      String userToken = pkInviteSentQuery.getUserToken();
      Integer skip = pkInviteSentQuery.getSkip();
      Integer limit = pkInviteSentQuery.getLimit();
      ...
      if (userToken == null || userToken.isEmpty()) {
        ...
        bodyParams.put("userId", pkInviteSentQuery.getUserId());
        bodyParams.put("reciverStreamId", pkInviteSentQuery.getReceiverStreamId());
        bodyParams.put("senderStreamId", pkInviteSentQuery.getSenderStreamId());

        Call<CommonResponse> call =
                retrofitManager.getUserService().sendInvitationToUserForPK(headers, bodyParams);
